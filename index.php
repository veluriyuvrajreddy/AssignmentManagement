 ...
 Entered Password or Username is wrong
  ...
  ?>
   ...
   <div class="card" >
    ...
    <p>Not Yet Registered ? <a href="registration.php">Click Here</a></p><br>
...
if (isset($_POST["submit"]))
{
    $name = $_POST["name"];
    $password = $_POST["pass"];
    $sql = "SELECT * FROM user_data WHERE name1='".$name."' AND password = '".$password."' ";
    $res = mysqli_query($db, $sql) ;
    $row = mysqli_fetch_assoc($res) ;
    if(mysqli_num_rows($res) == 1 )
    {
        $_SESSION["name"] = $row["name1"];
        header("location:".$domain."subject.php");
    }
    else header("location:".$domain."?error=wrong");
}
?>